...
TEST_F(VhpopPlannerTest, StartTimesNonDecreasing) {
  auto plan = planner_->generate_plan(simple_domain_obj_, simple_problem_obj_);
...
TEST_F(VhpopPlannerTest, FailedPlanHasNoTimingInfo) {
  auto plan =
      planner_->generate_plan(omni_plan::pddl::Domain(), simple_problem_obj_);
...
TEST_F(VhpopPlannerTest, ParallelPlanWithMultipleRobots) {
  ...
  if (plan.has_solution()) {
    ...
    // VHPOP may or may not produce parallel actions for this problem
