...
class SmtpPlannerTest : public ::testing::Test {
protected:
  void SetUp() override {
  ...
  std::unique_ptr<SmtpPlanner> planner_;
...
TEST_F(SmtpPlannerTest, ConstructorCreatesPlanner) {
...
TEST_F(SmtpPlannerTest, PlanHasStartTimes) {
  auto plan = planner_->generate_plan(simple_domain_obj_, simple_problem_obj_);

  if (plan.has_solution()) {
    for (size_t i = 0; i < plan.size(); ++i) {
      float start_time = plan.get_action_start_time(i);
      // SMTP produces temporal plans with start times >= 0
...
TEST_F(SmtpPlannerTest, ParallelPlanWithMultipleRobots) {
  ...
  if (plan.has_solution()) {
    ...
    // SMTP may or may not produce parallel actions for this problem
