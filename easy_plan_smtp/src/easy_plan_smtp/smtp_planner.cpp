...
easy_plan::pddl::Plan SmtpPlanner::get_plan(
  ...
  std::string command =
      ...
      "/planner/SMTPlan " + domain_file + " " + problem_file + " -u 1000 > " +
      output_file + " 2>&1";
  ...
  if (status != 0 || output.find("No plan found") != std::string::npos ||
      output.find("Critical Errors") != std::string::npos) {
