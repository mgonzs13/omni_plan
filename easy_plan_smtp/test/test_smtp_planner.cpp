...
protected:
  void SetUp() override { planner_ = std::make_unique<SmtpPlanner>(); }
...
TEST_F(SmtpPlannerTest, GetPlanWithEmptyActionsMap) {
  ...
  // Plan might succeed at SMTP level but actions won't be mapped
  // The behavior depends on SMTP availability
}
