...
class LpgPlannerTest : public ::testing::Test {
protected:
  void SetUp() override {
    rclcpp::init(0, nullptr);
    node_ = std::make_shared<rclcpp::Node>("test_node");
    planner_ = std::make_unique<LpgPlanner>();
  ...
  std::unique_ptr<LpgPlanner> planner_;
...
// Test: LpgPlanner constructor
TEST_F(LpgPlannerTest, ConstructorCreatesPlanner) {
  EXPECT_NE(planner_, nullptr);
}

// Test: generate_plan with invalid domain returns failed plan
TEST_F(LpgPlannerTest, GetPlanWithInvalidDomainReturnsFailed) {
...
TEST_F(LpgPlannerTest, GetPlanWithEmptyDomainReturnsFailed) {
  auto actions = create_actions();
  auto plan = planner_->generate_plan(omni_plan::pddl::Domain(),
...
TEST_F(LpgPlannerTest, GetPlanWithEmptyActionsMap) {
  std::unordered_map<std::string, std::shared_ptr<omni_plan::pddl::Action>>
      empty_actions;
...
TEST_F(LpgPlannerTest, GetPlanWithUnsolvableProblemReturnsFailed) {
  auto actions = create_actions();
...
TEST_F(LpgPlannerTest, FailedPlanHasSizeZero) {
  auto actions = create_actions();
  auto plan = planner_->generate_plan(omni_plan::pddl::Domain(),
                                      simple_problem_obj_, actions);
...
TEST_F(LpgPlannerTest, MultiplePlannerCalls) {
  auto actions = create_actions();
