...
class OpticPlannerTest : public ::testing::Test {
protected:
  void SetUp() override {
    rclcpp::init(0, nullptr);
    node_ = std::make_shared<rclcpp::Node>("test_node");
    planner_ = std::make_unique<OpticPlanner>();
  ...
  std::unique_ptr<OpticPlanner> planner_;
...
// Test: OpticPlanner constructor
TEST_F(OpticPlannerTest, ConstructorCreatesPlanner) {
  EXPECT_NE(planner_, nullptr);
}

// Test: generate_plan with invalid domain returns failed plan
TEST_F(OpticPlannerTest, GetPlanWithInvalidDomainReturnsFailed) {
  auto plan =
...
TEST_F(OpticPlannerTest, GetPlanWithEmptyDomainReturnsFailed) {
...
TEST_F(OpticPlannerTest, GetPlanWithUnsolvableProblemReturnsFailed) {
  auto plan =
...
TEST_F(OpticPlannerTest, FailedPlanHasSizeZero) {
  auto plan =
      planner_->generate_plan(omni_plan::pddl::Domain(), simple_problem_obj_);
...
TEST_F(OpticPlannerTest, MultiplePlannerCalls) {
  auto plan1 =
      planner_->generate_plan(omni_plan::pddl::Domain(), simple_problem_obj_);
  auto plan2 =
      planner_->generate_plan(omni_plan::pddl::Domain(), simple_problem_obj_);
...
// Integration test: Valid domain and problem (requires OPTIC to be installed)
TEST_F(OpticPlannerTest, ValidDomainAndProblemReturnsPlan) {
  const std::string optic_binary =
      omni_plan::utils::get_package_share_path("omni_plan_optic") +
      "/bin/optic-clp";
  if (access(optic_binary.c_str(), X_OK) != 0) {
    GTEST_SKIP() << "OPTIC binary not executable at " << optic_binary;
  }

  auto plan = planner_->generate_plan(simple_domain_obj_, simple_problem_obj_);

  // The fixture is known-solvable: a missing solution means OPTIC is broken.
  ASSERT_TRUE(plan.has_solution()) << "OPTIC failed to solve the fixture";
