...
#include "easy_plan_val/val_validator.hpp"

using namespace easy_plan_val;
...
std::string ValValidator::parse_pddl(const easy_plan::pddl::Plan &plan) const {
...
bool ValValidator::validate_plan(const std::string &domain_path,
  ...
  std::string command =
      ament_index_cpp::get_package_share_directory("easy_plan_val") +
      "/bin/validate";
...
PLUGINLIB_EXPORT_CLASS(ValValidator, easy_plan::PlanValidator)